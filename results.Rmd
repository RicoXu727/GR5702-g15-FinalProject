...
plot_ly(Sorted_data, x = ~total_amount, y = ~CatName,
                      ...
                      marker = list(opacity = 0.7,sizemode = "diameter")) %>% add_markers()%>%layout(title = 'Top 20 Wine Sale in Iowa', xaxis = list(title = 'Total_sales'), yaxis = list(title = 'Category Name'), legend=list(title=list(text='<b>Trend</b>')))
```

The graph shows that Iowa residents have a clear preference for American Vodkas and Canadian Whiskies. These two wines are far ahead of other brands in terms of sales. All other wine sales are concentrated within a range of approximately 1M. Secondly, the factor of price does not play a very significant role in the consumption of alcohol. The high sales volume like Canadian Whiskies, it is not cheap the average price is 14.439. Some cheaper wines, such as American Dry Gin, Blended Whiskies, are no more popular than 100% Agave Tequila.
...
Then, we wanted to know which wines are good for bulk purchases as well as family gatherings in Iowa. Is the reason for the high sales in the previous chart a large volume purchase?
...
stackg <- ggplot(filtered_data, aes(fill = BottleSale, y=BScount, x=CatName)) + 
    ...
    ggtitle("Wine purchased in large quantities") +
...
For demonstration purposes, only the top three best-selling alcohol categories for the top 30 counties.

Among 53 different alcohol types, nine appeared to be the top-selling category for the selected counties. In addition, whisky and vodka are frequently among the top-selling categories for the selected counties.
...
The map shows each county's alcohol sales (in dollars) with dark blue being the county with the highest dollar sales.

We notice that counties with higher sales are most likely to include larger cities, suggesting that alcohol sales are proportional to county population.
...
There is a slight increase in the total amount of bottles sold in all categories of alcohol from January to December in 2019.
...
The plot shows that people in Iowa do not purchase as much alcohol in the first quarter in 2019 as they do in the other quarters.
...
It is initially assumed that the consumption of alcohol during the first quarter of the year is higher since the weather is cold, which contradicts what the data illustrates. We speculate the reason is that people tend not to go out as often due to the cold and snowy weather.
...
There is no observable difference in the trend in alcohol sales among weekdays.
...
However, the sales of alcohol during the holiday seasons is not as high as expected since not all the stores open during holidays and people tend to prepare alcohol before holidays.
